   ...
   if (fctl) {
      ...
      if (strcmp(opt, optstr[iopt]) == 0){ //SW
               ...
               case (34): //OU, clocl=4|40
                  ...
                  if (data.reversionpara[2] <= 0) data.reversionpara[2] = 1;
                  break;
               }
               break;
            }
   ...
   if (com.clock > 1) {               /* sigma2, rates for nodes or branches */
      ...
      for (i = 0; i < g1; i++){
         ...
         if(com.clock == 4 || com.clock == 40){ //OU
            data.reversion[i] = smallr + rndgamma(data.reversionpara[0]) / data.reversionpara[1];
         } else { //GBM
      ...
      if (com.clock == 3 || com.clock == 30 || com.clock == 31 || com.clock == 32){
         ...
         for (i = 0; i < g1; i++) {
            ...
            if (firsttime && fout) {
               ...
               else             fprintf(fout, "\treversion");
            }
            x[np++] = data.reversion[i]; //not log
         }
      }
   ...
   */
   ...
   for (i = 0; i < nfs; i++) {
      ...
      if (j != root && stree.nodes[j].fossil == LOWER_F) {
         ...
         s = a*c*sNormal;
         A = 0.5 + 1 / Pi*atan(p / c);
         CLower[i][0] = (1 / tailL - 1) / (Pi*A*c*(1 + (p / c)*(p / c)));  /* reversionCauchy */
         CLower[i][1] = (1 / tailL - 1) * a / (s*sqrt(Pi / 2));            /* reversionNormal  */
   ...
   /* This calculates the log of the prior of branch rates under the rate-drift models.
      ...
      clock=4/40:
         OU process on log-rates:
            dY_t = reversion * (mu - Y_t) dt + sigma dW_t
         with
      ...
      for (inode = 0; inode < stree.nnode; inode++) {
         ...
         for (locus = 0; locus < g; locus++) {
            ...
            lnpR += -zz * zz / (2.0 * data.sigma2[locus]) - log(r);
             ...
             com.clock == 32 || com.clock == 4 || com.clock == 40) &&
         ...
         for (locus = 0; locus < g; locus++) {
            ...
            if (com.clock == 3 || com.clock == 30 || com.clock == 31 ||
               ...
               /* OU model on log-rates
                  Assumptions:
                  - reversion is stored in data.drift[locus]
                  - mu = log(data.rgene[locus])
               */
               double reversion, sigma2, mu;
               ...
               if (reversion < 0)
                  zerror("OU reversion must be >= 0 in lnpriorRates().");
               ...
               if (fabs(reversion) < 1e-12) {
                  ...
                  m1 = (y0 - mu) * exp(-reversion * (tA + t1)) + mu;
                  m2 = (y0 - mu) * exp(-reversion * (tA + t2)) + mu;
   ...
   /* This calculates the lnpriorRatio when one rate is changed.
      ...
      clock=4|40:
         OU model on log-rates, using
            reversion = data.drift[locus]
            mu    = log(data.rgene[locus])
   */
   double rnew = stree.nodes[inodeChanged].rates[locus], lnpRd = 0, a, b, z, znew;
   double zz, rA, r1, r2, y1, y2, t, tA, t1, t2, drift, reversion;
