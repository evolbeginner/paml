...
int UpdateParaRates(double *lnL, double steplength[], char accept[], double space[])
{
   ...
   */
   ...
   for (ip = 0; ip < np; ip++) {  /* mu (rgene) and sigma2 for each locus and for overall */
      ...
      else if (ip == 2)        { para = data.drift; gD = data.sigma2para; } /* drift */
	 ...
	 //pold = (ip == 2) ? exp(para[locus]) : para[locus];//SW
	 if(ip < 2){
	 	pold = para[locus];
		y = log(pold);
		 ynew = y + e*rndSymmetrical();
	 ...
	 if(ip < 2){
         ...
         lnacceptance = (ip == 2) ? 0.0 : (ynew - y); //SW
	printf("999\t%f\t%f\t%f\n", pold, ynew, y);
   ...
   */
   ...
   if (com.clock >= 3 && data.priorrate == 1)
      ...
      for (inode = 0; inode < stree.nnode; inode++) {
		...
		//SW
		switch (com.clock) {
			case 3: data.drift[locus] = 0; break;
			case 31: data.drift[locus] = data.sigma2[locus]/2; break;
